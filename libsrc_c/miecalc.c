/*--------------------------------------------------------------------
 * $Id: miecalc.c 3153 2015-08-06 20:20:48Z claudia.emde $
 * 
...
/* CE, the following two lines can be deleted... */
  ...
  if (min_columns==1 && max_columns==1) {
    ...
    *alloc_moments = 1;
    ...
    (void) fgets (dummy, 1024, file);           /* first line ignored          */

    if ( ! fscanf (file, "%lf", wavelen) ) {      /* 2nd line, wavelength        */
      fprintf (stderr, "Error reading file %s\n", filename);
      return -1;
    }
    (void) fgets (dummy, 1024, file);           /* ignore rest of line         */
    ...
    (void) fgets (dummy, 1024, file);           /* 4th line, distflag, ignored */
    
    (void) fgets (dummy, 1024, file);           /* 5th line, alpha, ignored    */
			   ...
			   size_t *nlam, double **wavelen, int quiet)
{
  ...
  if (min_columns==1 && max_columns==1) {
    **wavelen = -999;  
    ...
    (void) fgets (dummy, 1024, f);           /* first line ignored          */

    if ( ! fscanf (f, "%lf", *wavelen) ) {     /* 2nd line, wavelength        */
      fprintf (stderr, "Error reading file %s\n", filename);
      return -1;
    }
    (void) fgets (dummy, 1024, f);           /* ignore rest of line         */
