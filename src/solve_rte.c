/*--------------------------------------------------------------------
 * $Id: solve_rte.c 3165 2015-08-20 12:51:42Z Claudia.Emde $
 * 
	...
	if (input.rte.solver == SOLVER_POLRADTRAN) {
	  for(lu=0; lu<output->atm.nzout; lu++) {
		...
		for (iu=0;iu<input.rte.numu;iu++) {
          ...
          output->albmed_r [iv] += rte_outband->albmed[0];
          output->trnmed_r [iv] += rte_outband->trnmed[0];
        }
        else {
	  for(lu=0; lu<output->atm.nzout; lu++) {
	    output->rfldir_r [lu][iv] += output->ipaweight[ipa] * rte_outband->rfldir [lu];
                  ...
                  double **p_rho_mass_zout, float **p_k_abs_layer, float **p_k_abs, float **p_k_abs_outband)
{
  ...
  output->albmed_r = (float *) calloc( output->wl.nlambda_r, sizeof(float) );
  output->trnmed_r = (float *) calloc( output->wl.nlambda_r, sizeof(float) );
  ...
  output->albmed = (float *) calloc( nlambda, sizeof(float) );
  output->trnmed = (float *) calloc( nlambda, sizeof(float) );
...
static int setup_and_call_solver (input_struct input, output_struct *output, rte_input *rte_in, rte_output *rte_out, 
				  ...
				  int iv, int ib, int ir, int *threed, int mc_loaddata)
{
  int status=0, lu=0, iv1=0, iv2=0, iv_alis=0, isp=0;
  static int first = 1;
  ...
  if (input.ck_scheme==CK_LOWTRAN) {
    ...
    status = sbdart_profile (output->atm.microphys.temper, output->atm.microphys.press, output->atm.zd, 
			     output->atm.microphys.dens[MOL_H2O], output->atm.microphys.dens[MOL_O3], 
			     -1.0, 
			     ...
			     input.ck_abs[CK_ABS_O4], input.ck_abs[CK_ABS_N2], input.ck_abs[CK_ABS_CO], 
			     input.ck_abs[CK_ABS_SO2], input.ck_abs[CK_ABS_NH3], input.ck_abs[CK_ABS_NO], input.ck_abs[CK_ABS_HNO3],
	...
	output->crs_ck.profile[0][iv].crs[lu][ib];
			...
			int *threed, int mc_loaddata, int verbose)
{
  ...
  case SOLVER_MONTECARLO:
	...
	return -1;
      ...
      status = mystic (&output->atm.nlyr,
		       input.n_caoth+2,
		       output->mc.dt, output->mc.om, output->mc.g1, output->mc.g2, output->mc.ff,
		       output->mc.ds,
                       &(output->mc.alis),
                       output->mc.refind,
		       ...
		       input.rte.mc.filename[FN_MC_RPV_SPECTRAL], output->rpv_surf.label, &(output->rpv_surf.n),
  ...
  case SOLVER_DISORT:
	...
	if ( ir == 0 && input.raman_fast ) {
	  ...
	  if ( ib == 0 ) {
	    ...
	    for (lu=0;lu<output->atm.nlev;lu++){
	      ...
	      status = crs_raman_N2(output->wl.lambda_r[iv], input.n_raman_transitions_N2,
				    output->atm.microphys.temper[lu], &tmp_crs[lu], &ivi, verbose);
	  ...
	  if ( input.raman_fast ) {
	    ...
	    status = set_raman_source(rte_in->qsrc, rte_in->qsrcu, input.rte.maxphi,
				      ...
				      output->atm.microphys.dens[MOL_AIR], 
	...
	ds_in.phi[iu] = (double) input.rte.phi[iu];
      ...
      for (lu=0; lu<=output->atm.nlyr; lu++) {
	if ( rte_in->planck )
	  ds_in.temper[lu] = (double) output->atm.microphys.temper[lu];
      }    
      if ( ds_in.flag.spher  ) {
	for (lu=0; lu<=output->atm.nlyr; lu++) {
  ...
  case SOLVER_TZS:
    ...
    status = c_tzs (output->atm.nlyr, output->dtauc, output->atm.nlev_common, output->atm.zd_common, 
		    ...
		    output->ssalb, output->atm.microphys.temper, 
  ...
  case SOLVER_SSS:
    ...
    F77_FUNC  (sss, SSS)(&output->atm.nlyr, output->dtauc, output->ssalb, &output->atm.nmom,
		   sss_pmom, output->atm.microphys.temper, &output->wl.wvnmlo_r[iv],
		   &output->wl.wvnmhi_r[iv], &rte_in->usrtau, &output->atm.nzout, rte_in->utau,
		   &input.rte.nstr, &rte_in->usrang, &input.rte.numu, input.rte.umu, 
		   &input.rte.nphi, input.rte.phi, &rte_in->ibcnd, &rte_in->fbeam, 
		   &rte_in->umu0, &output->atm.phi0_r[iv], &rte_in->fisot, &rte_in->lamber, 
  ...
  case SOLVER_FTWOSTR:
    ...
    F77_FUNC  (twostr, TWOSTR)(&output->alb.albedo_r[iv], &(rte_in->btemp), &input.rte.deltam,
		      ...
		      output->atm.microphys.temper, &(rte_in->ttemp), &(rte_in->umu0), 
  ...
  case SOLVER_TWOSTR:
    ...
    for (lu=0; lu<=output->atm.nlyr; lu++) {
      ...
      if ( rte_in->planck )
	twostr_ds.temper[lu] = (double) output->atm.microphys.temper[lu];
    }    
...
static int init_rte_input (rte_input *rte, input_struct input, output_struct *output)
{
  ...
  case SRC_THERMAL:
    ...
    rte->ttemp = output->atm.microphys.temper[0];
  ...
  case SOLVER_POLRADTRAN:
    ...
    for (lu=0;lu<=output->atm.nlyr;lu++) {
      ...
      rte->pol.temperatures[lu] = (double) output->atm.microphys.temper[lu];
    }
    break;
  case SOLVER_TWOSTR:
  case SOLVER_FTWOSTR:
...
/*   } */
  ...
  for (lc=0; lc<output->atm.nlev-1; lc++) {
    ...
    taum  = output->atm.optprop.tau_molabs_r[lc][iv][iq];
    ...
    case CK_FU:
      taur = output->atm.optprop.tau_rayleigh_r[lc][iv][iq];
      break;
    ...
    case CK_RAMAN:
      taur = output->atm.optprop.tau_rayleigh_r[lc][iv][0];
      break;
...
static int set_raman_source(double ***qsrc, double ***qsrcu, int maxphi, int nlev, int nzout, 
  ...
  if ( test ) {
	...
	//aky	fprintf(stderr,"gabba %7.3f %13.8e\n", tmp_zd[lub], tmpsum);
	verbose = 0;
	if (verbose && maz == 0) {
	  crs_RL_tot = 0;
	  crs_RG_tot = 0;
	...
	for (iq=0;iq<nstr;iq++) {
	  ...
	  for (is=0;is<n_shifts;is++) {
	    ...
	    //aky	    fprintf(stderr,"gabba wanted_wl  %9.4f %3d %9.4f %9.4f\n", wanted_wl, is, wl_shifts[is], lambda_fact);
	    sum = 0;
	    for (jq=0;jq<nstr;jq++) {
