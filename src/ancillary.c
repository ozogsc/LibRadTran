/*--------------------------------------------------------------------
 * $Id: ancillary.c 3128 2015-06-27 13:35:11Z bernhard.mayer $
 * 
                    ...
                    char *filename)
{
  ...
  if (strlen(input.filename[FN_REPTRAN])>0)
    ...
    else if(input.ck_scheme == CK_REPTRAN_CHANNEL){
      len=strlen(input.ck_reptran_channel)-strlen(strchr(input.ck_reptran_channel,'_')); // length of channel name until underscore
      if(isdigit(input.ck_reptran_channel[len-1])) len=len-1; 
      strncat (filename, input.ck_reptran_channel, len);  // first part of channel name without first underscore or digit
    }
  }
...
int interpolate_atmosphere (float *z, float **p_p, float **p_T, float ***p_dens, float **p_Tavg, float ***p_densavg, int n,  
			    float *znew, int nnew,
...

{
  ...
  /* Pressure */
  status=0;
  status += interpolate_profile (z, p_p, n, znew, nnew, interpol_method_press, quiet);

  /* Temperature */
  status += interpolate_profile (z, p_T, n, znew, nnew, interpol_method_temper, quiet);

  /* copy old air number density */
  dens_air_old = calloc (n, sizeof(float));
  for (lc=0; lc<n; lc++)
    dens_air_old[lc] = (*p_dens)[MOL_AIR][lc];


  /* Air */
  /* this is not 100% consistent with interpolation of p and T */
  status += interpolate_profile (z, &((*p_dens)[MOL_AIR]), n, znew, nnew, interpol_method_gas[MOL_AIR], quiet);
...
/*     (*p_dens)[MOL_AIR][lc] =  (*p_p)[lc] / (boltzmann * (*p_T)[lc]) * 100.0 / 1e6; */
  ...
  for (i=0; i<MOL_NN; i++) {
    if ( i != MOL_AIR )
      status += interpolate_density (z, &((*p_dens)[i]),  n, znew, nnew, interpol_method_gas[i],
                                     dens_air_old, (*p_dens)[MOL_AIR], quiet);
  }
  ...
  status += average_dens (*p_T, (*p_dens)[MOL_AIR], 
			  znew, nnew,
  ...
  if (*p_densavg!=NULL) {
	...
	free ((*p_densavg)[i]);
      ...
      status += average_dens ((*p_dens)[i], (*p_dens)[MOL_AIR],
			      znew, nnew,
...
int convolve (input_struct input, output_struct *output)
{
  ...
  if (input.rte.solver == SOLVER_POLRADTRAN) {
    ...
    /* albmed */
    status = cnvlv (x_data, output->albmed, output->wl.nlambda_h,
                    x_slit, y_slit, n_slit, 0);
    ...
    /* trnmed */
    status = cnvlv (x_data, output->trnmed, output->wl.nlambda_h,
                    x_slit, y_slit, n_slit, 0);
    if (status!=0)  { 
...
int spline_interpolate(input_struct input, output_struct *output)
{
  ...
  if (input.rte.solver == SOLVER_POLRADTRAN) {
    ...
    /* albmed */
    status = arb_wvn(output->wl.nlambda_h, output->wl.lambda_h, output->albmed,  
                     output->wl.nlambda_s, output->wl.lambda_s, output->albmed, 
                     linear, 0);
    ...
    /* trnmed */
    status = arb_wvn(output->wl.nlambda_h, output->wl.lambda_h, output->trnmed,  
                     output->wl.nlambda_s, output->wl.lambda_s, output->trnmed, 
                     linear, 0);
...
int internal_to_transmittance_grid (input_struct input, output_struct *output)
{
  ...
  if (output->wl.use_reptran==0) {
    ...
    if (input.rte.solver != SOLVER_POLRADTRAN) {
      ...
      output->albmed_t = calloc(output->wl.nlambda_t, sizeof(float));
      output->trnmed_t = calloc(output->wl.nlambda_t, sizeof(float));

      status += weighting_reptran(&output->wl, 0, output->albmed_r, output->albmed_t);
      status += weighting_reptran(&output->wl, 0, output->trnmed_r, output->trnmed_t);
...
int interpolate_transmittance (input_struct input, output_struct *output)
{
  ...
  if (output->wl.nlambda_t == 1 || output->wl.use_reptran == 1 || (output->wl.use_reptran == 0 && input.ck_scheme!=CK_CRS && input.ck_scheme!=CK_RAMAN && input.ck_scheme!=CK_LOWTRAN)) {
	...
	iv = output->wl.map_e2h[ivh];
      ...
      if (input.rte.solver != SOLVER_POLRADTRAN) {
        output->albmed  [ivh] = output->albmed_t  [iv];
        output->trnmed  [ivh] = output->trnmed_t  [iv];
      }
    ...
    if (input.rte.solver != SOLVER_POLRADTRAN) {
      status += arb_wvn(output->wl.nlambda_t, output->wl.lambda_t, output->albmed_t, 
                        output->wl.nlambda_h, output->wl.lambda_h, output->albmed, linear, 0);
      status += arb_wvn(output->wl.nlambda_t, output->wl.lambda_t, output->trnmed_t, 
                        output->wl.nlambda_h, output->wl.lambda_h, output->trnmed, linear, 0);
    }
			...
			int            skip_optical_properties )
{
  ...
  if (first!=0) {
    ...
    if (input.rte.solver == SOLVER_MONTECARLO) {
      ...
      output->mc.temper  = (float *) calloc (nlev, sizeof(float));
  ...
  if (!skip_optical_properties) {
	...
	for (isp=CAOTH_FIR; isp<n_caoth; isp++) {
      ...
      switch(input.ck_scheme) {
      case CK_FU:
        bsca_s[CAOTH_MOL] = output->atm.optprop.tau_rayleigh_r[lc][iv][iq];
        break;
      ...
      case CK_REPTRAN:
      case CK_REPTRAN_CHANNEL:
        bsca_s[CAOTH_MOL] = output->atm.optprop.tau_rayleigh_r[lc][iv][0];
        break;

      case CK_RAMAN:
	if ( (input.raman&&!input.raman_fast) ||  (input.raman_fast && ir==1) )
	  bsca_s[CAOTH_MOL] = linpol(wvl1, wvl2, output->atm.optprop.tau_rayleigh_r[lc][iv1r][0],
				    output->atm.optprop.tau_rayleigh_r[lc][iv2r][0], wvl);
	else
	  bsca_s[CAOTH_MOL] = output->atm.optprop.tau_rayleigh_r[lc][iq][0];
        break;
      ...
      case MOLABS_LOOKUP:
        if ( (input.raman&&!input.raman_fast) ||  (input.raman_fast && ir==1) )
	  babs_s[CAOTH_MOL] = linpol(wvl1, wvl2, output->atm.optprop.tau_molabs_r[lc][iv1r][0], 
                                     output->atm.optprop.tau_molabs_r[lc][iv2r][0], wvl);
	else if ( input.raman_fast && ir==0 )
	  babs_s[CAOTH_MOL] = output->atm.optprop.tau_molabs_r[lc][iv][0];
	else
          babs_s[CAOTH_MOL] = output->atm.optprop.tau_molabs_r[lc][iv][iq];
        if ( input.rte.solver == SOLVER_SDISORT ) 
	  babs_mol_md = output->atm.optprop.tau_molabs_md_r[lc][iv][iq];
	else
	  babs_mol_md = 0.0;
        break;
	...
	if ( (input.raman&&!input.raman_fast) ||  (input.raman_fast && ir==1) )
	  ...
	  babs_s[CAOTH_MOL] = output->atm.optprop.tau_molabs_r[lc][iq][0];
	else
	  babs_s[CAOTH_MOL] = output->atm.optprop.tau_molabs_r[lc][iv][iq];
	...
	output->mc.temper[nlyr-lc] = output->atm.microphys.temper[lc];
      }
   ...
   if ((retval = nc_create("test.optical_properties.nc", NC_CLOBBER, &ncid)))	ERR(retval);
	...
	if (output->mc.alis.om!=NULL) {
   	...
   	if ((retval = write_netcdf_2Dfloat(ncid, output->mc.re, n_caoth, nlev, "output->re", "ncaoth", "nlev")))	ERR(retval);
    	...
    	if ((retval = write_netcdf_1Dfloat(ncid, output->mc.temper, nlev, "output->mc.temper", "nlev")))	ERR(retval);
...
{ 
  ...
  for (iv=0; iv<output->wl.nlambda_h; iv++) {
    output->albmed_int +=  (double) output->albmed [iv];
    output->trnmed_int +=  (double) output->trnmed [iv];
  }
...
{ 
  ...
  /* fluxes and radiances */
  for (iv=0; iv<output->wl.nlambda_h; iv++) yint[iv] = output->albmed[iv];
  output->albmed_int  = integrate(xint, yint, output->wl.nlambda_h);
  for (iv=0; iv<output->wl.nlambda_h; iv++) yint[iv] = output->trnmed[iv];
  output->trnmed_int   = integrate(xint, yint, output->wl.nlambda_h);
  ...
  case MCABS_UNIT_K_PER_DAY:
	...
	if (output->atm.threed[ks]) {   /* only for 3D layers, BM07122005 */
	  ...
	  if (input.rte.mc.abs_unit == MCABS_UNIT_K_PER_DAY) {
	    rho_air = output->atm.microphys.dens_avg[MOL_AIR][output->atm.Nzcld-1-ks] * 1.e+6 * 1.e-3 * input.atm.mol_mass[MOL_AIR] / avogadro;
	    /* 1.e+6: convert from cm-3 to m-3; 1.e-3: convert g -> kg */
	    status = specific_heat_capacity_moist_air(output->atm.microphys.temper_avg[output->atm.Nzcld-1-ks],
						      output->atm.microphys.dens_avg[MOL_AIR][output->atm.Nzcld-1-ks],
						      output->atm.microphys.dens_avg[MOL_H2O][output->atm.Nzcld-1-ks],
						      &(c_p), input.quiet);
	    if (status != 0) {
	...
	if (input.rte.mc.abs_unit == MCABS_UNIT_K_PER_DAY) {
	  rho_air = output->atm.microphys.dens_avg[MOL_AIR][lc-1] * 1.e+6 * 1.e-3 * input.atm.mol_mass[MOL_AIR] / avogadro;
	  /* 1.e+6: convert from cm-3 to m-3; 1.e-3: convert g -> kg */
	  status = specific_heat_capacity_moist_air(output->atm.microphys.temper_avg[lc-1],
						    output->atm.microphys.dens_avg[MOL_AIR][lc-1],
						    output->atm.microphys.dens_avg[MOL_H2O][lc-1],
						    &(c_p), input.quiet);
	  if (status != 0) {
	    fprintf (stderr,"Error, calculating 'c_p' of moist air in %s (%s)\n", function_name, file_name);
	      return -1;
	...
	if (!input.quiet && input.ipa3d!=1) /*ulrike added && input.ipa3d!=1*/
	  fprintf (stderr, "converting to heating rate, level %d %.3f - %.3f km, dens=%g, temper=%.3f, scale_factor %f\n", 
		   lc, output->atm.zd[lc], output->atm.zd[lc-1], output->atm.microphys.dens_avg[MOL_AIR][lc-1], output->atm.microphys.temper_avg[lc-1], scale_factor_abs3d);
  ...
  for (ks=0; ks<output->atm.nzout; ks++)
	...
	if (output->wl.nlambda_h > 0) {
	  ...
	  output->uavgup3d   [ks][is][js][0] *= ffactor;
          ...
          for (ip=0; ip<input.rte.mc.nstokes; ip++)
            for (ic=0; ic<output->mc.alis.Nc; ic++)
              output->radiance3d [ks][is][js][0][ip][ic] *= rfactor;
	  ...
	  if (input.rte.mc.std) {
	    ...
	    for (ip=0; ip<input.rte.mc.nstokes; ip++)
	      output->radiance3d_var [ks][is][js][0][ip] *= rfactor2;
...
int double2float_integrated_values(input_struct input, output_struct *output)
{
  ...
  if (input.rte.solver == SOLVER_POLRADTRAN) {  /* polRadtran output */
    ...
    output->albmed[0] = (float) output->albmed_int;
    output->trnmed[0] = (float) output->trnmed_int;
...
int average_dens(float *dens, float *dens_air, float *zd, int nlev, int interpol_method, float **dens_avg, int allocate)
{
  ...
  case INTERP_METHOD_LINEAR:
    ...
    break;
  case INTERP_METHOD_LOG:
    for (lc=0; lc<n_layer; lc++) (*dens_avg)[lc] = log_average(dens[lc],dens[lc+1]);
    break;
