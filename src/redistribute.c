/*--------------------------------------------------------------------
 * $Id: redistribute.c 3121 2015-05-29 15:22:43Z josef.gasteiger $
 * 
...
/*   } */
  ...
  if (input.verbose) {
	...
	babso_o3 = 0.5*deltaz * (output->atm.microphys.dens[MOL_O3][lc]*output->crs.crs[lc][MOL_O3][iv]
				 + output->atm.microphys.dens[MOL_O3][lc+1]*output->crs.crs[lc+1][MOL_O3][iv]);
	if (output->atm.microphys.denstab_id != MOL_O3) babso_md += babso_o3;
	vcol_o3    += 0.5*deltaz * (output->atm.microphys.dens[MOL_O3][lc] + output->atm.microphys.dens[MOL_O3][lc+1]);
	

	babso_co2 = 0.5*deltaz * (output->atm.microphys.dens[MOL_CO2][lc]*output->crs.crs[lc][MOL_CO2][iv]
				 + output->atm.microphys.dens[MOL_CO2][lc+1]*output->crs.crs[lc+1][MOL_CO2][iv]);
	if (output->atm.microphys.denstab_id != MOL_CO2) babso_md += babso_co2;
	vcol_co2    += 0.5*deltaz * (output->atm.microphys.dens[MOL_CO2][lc] + output->atm.microphys.dens[MOL_CO2][lc+1]);
	

	babso_o2 = 0.5*deltaz * (output->atm.microphys.dens[MOL_O2][lc]*output->crs.crs[lc][MOL_O2][iv]
				 + output->atm.microphys.dens[MOL_O2][lc+1]*output->crs.crs[lc+1][MOL_O2][iv]);
	if (output->atm.microphys.denstab_id != MOL_O2) babso_md += babso_o2;
	vcol_o2    += 0.5*deltaz * (output->atm.microphys.dens[MOL_O2][lc] + output->atm.microphys.dens[MOL_O2][lc+1]);
	

	babso_no2 = 0.5*deltaz * (output->atm.microphys.dens[MOL_NO2][lc]*output->crs.crs[lc][MOL_NO2][iv]
				  + output->atm.microphys.dens[MOL_NO2][lc+1]*output->crs.crs[lc+1][MOL_NO2][iv]);
	if (output->atm.microphys.denstab_id != MOL_NO2) babso_md += babso_no2;
	vcol_no2   += 0.5*deltaz * (output->atm.microphys.dens[MOL_NO2][lc] + output->atm.microphys.dens[MOL_NO2][lc+1]);
	

	babso_bro = 0.5*deltaz * (output->atm.microphys.dens[MOL_BRO][lc]*output->crs.crs[lc][MOL_BRO][iv]
				  + output->atm.microphys.dens[MOL_BRO][lc+1]*output->crs.crs[lc+1][MOL_BRO][iv]);
	if (output->atm.microphys.denstab_id != MOL_BRO) babso_md += babso_bro;
	vcol_bro   += 0.5*deltaz * (output->atm.microphys.dens[MOL_BRO][lc] + output->atm.microphys.dens[MOL_BRO][lc+1]);
	

	babso_oclo = 0.5*deltaz * (output->atm.microphys.dens[MOL_OCLO][lc]*output->crs.crs[lc][MOL_OCLO][iv]
				   + output->atm.microphys.dens[MOL_OCLO][lc+1]*output->crs.crs[lc+1][MOL_OCLO][iv]);
	if (output->atm.microphys.denstab_id != MOL_OCLO) babso_md += babso_oclo;
	vcol_oclo  += 0.5*deltaz * (output->atm.microphys.dens[MOL_OCLO][lc] + output->atm.microphys.dens[MOL_OCLO][lc+1]);
	

	babso_hcho = 0.5*deltaz * (output->atm.microphys.dens[MOL_HCHO][lc]*output->crs.crs[lc][MOL_HCHO][iv]
				   + output->atm.microphys.dens[MOL_HCHO][lc+1]*output->crs.crs[lc+1][MOL_HCHO][iv]);
	if (output->atm.microphys.denstab_id != MOL_HCHO)  babso_md += babso_hcho;
	vcol_hcho  += 0.5*deltaz * (output->atm.microphys.dens[MOL_HCHO][lc] + output->atm.microphys.dens[MOL_HCHO][lc+1]);
	...
	babso_amftot  += babso_amf;
      ...
      fprintf (stderr, "AMF BOA crs   %4.0f | %12.6e | %12.6e | %12.6e | %12.6e | %12.6e | %12.6e | %12.6e\n", 
	       0.0/0.0,
	       output->crs.crs[output->atm.nlyr-1][MOL_O3]  [iv], 
	       output->crs.crs[output->atm.nlyr-1][MOL_O2] [iv], 
	       output->crs.crs[output->atm.nlyr-1][MOL_CO2]  [iv], 
	       output->crs.crs[output->atm.nlyr-1][MOL_NO2] [iv], 
	       output->crs.crs[output->atm.nlyr-1][MOL_BRO] [iv], 
	       output->crs.crs[output->atm.nlyr-1][MOL_OCLO][iv],
	       output->crs.crs[output->atm.nlyr-1][MOL_HCHO][iv]);
      fprintf (stderr, " ---------------------------------------------------------------------------------------------------------------------------\n");
				...
				float *zd_common, int nlyr_common)
{
  ...
  for (lc=0; lc<nlyr; lc++) 
    for (id=0; id<nmol; id++)
      for (iv=nlambda_lower; iv<=nlambda_upper; iv++)
	tmpcrs[lc][id][iv] = crs->crs[lc][id][iv];
  
  /* free original ary and allocate with changed resolution */
  ASCII_free_float_3D (crs->crs, nlyr+1, nmol);
  if ((status = ASCII_calloc_float_3D(&crs->crs, nlyr_common+1, nmol, nlambda)) != 0) {
...
int redistribute_molecular (atm_optprop_struct *optprop, int nlambda, 
			    ...
			    float *zd_common, int nlyr_common)
{
  int lc=0, iv=0, iq=0, scale=0, status=0;
  double ***tmpopt=NULL;
  ...
  /* Allocate memory for temporary ary */
  if ((status = ASCII_calloc_double_3D_arylen_restricted (&tmpopt, nlyr, 
  ...
  for (lc=0; lc<nlyr; lc++) 
	...
	tmpopt[lc][iv][iq] = optprop->tau_molabs_r[lc][iv][iq];
  
  /* free original ary and allocate with changed resolution */
  ASCII_free_double_3D (optprop->tau_molabs_r, nlyr, nlambda);
  if ((status = ASCII_calloc_double_3D_arylen_restricted (&optprop->tau_molabs_r, nlyr_common, 
							  nlambda, nlambda_lower, nlambda_upper, nq)) != 0) {
  ...
  for (iv=nlambda_lower; iv<=nlambda_upper; iv++) {
    for (iq=0; iq<nq[iv]; iq++) {
      xin[0] = zd[0];
      yin[0] = tmpopt[0][iv][iq];
      for (lc=0; lc<nlyr; lc++) {
	xin[lc+1] = zd[lc+1];
	yin[lc+1] = tmpopt[lc][iv][iq];
      }
	...
	for (lc=1; lc<=nlyr_common; lc++) 
	  optprop->tau_molabs_r[lc-1][iv][iq] = yout[lc];      
    }
  }

  ASCII_free_double_3D (tmpopt, nlyr, nlambda);
  ...
  for (lc=0; lc<nlyr; lc++) 
    for (iv=nlambda_lower; iv<=nlambda_upper; iv++)
      for (iq=0; iq<nq[iv]; iq++) 
	tmpopt[lc][iv][iq] = optprop->tau_molabs_md_r[lc][iv][iq];
  
  /* free original ary and allocate with changed resolution */
  ASCII_free_double_3D (optprop->tau_molabs_md_r, nlyr, nlambda);
  if ((status = ASCII_calloc_double_3D_arylen_restricted (&optprop->tau_molabs_md_r, nlyr_common, 
							  nlambda, nlambda_lower, nlambda_upper, nq)) != 0) {
  ...
  for (iv=nlambda_lower; iv<=nlambda_upper; iv++) {
	...
	for (lc=1; lc<=nlyr_common; lc++) 
	  optprop->tau_molabs_md_r[lc-1][iv][iq] = yout[lc];      
    }
  }
  ASCII_free_double_3D (tmpopt, nlyr, nlambda);
  ...
  for (lc=0; lc<nlyr; lc++) 
    for (iv=nlambda_lower; iv<=nlambda_upper; iv++)
      for (iq=0; iq<nq[iv]; iq++) 
	tmpopt[lc][iv][iq] = optprop->tau_rayleigh_r[lc][iv][iq];
  
  /* free original ary and allocate with changed resolution */
  ASCII_free_double_3D (optprop->tau_rayleigh_r, nlyr, nlambda);
  if ((status = ASCII_calloc_double_3D_arylen_restricted (&optprop->tau_rayleigh_r, nlyr_common, 
							  nlambda, nlambda_lower, nlambda_upper, nq)) != 0) {
    fprintf (stderr, "Error allocating memory for tmpopt\n");
    return status;
  }
  ...
  for (iv=nlambda_lower; iv<=nlambda_upper; iv++) {
	...
	for (lc=1; lc<=nlyr_common; lc++) 
	  optprop->tau_rayleigh_r[lc-1][iv][iq] = yout[lc];      
    }
  }
  ASCII_free_double_3D (tmpopt, nlyr, nlambda);
