/*--------------------------------------------------------------------
 * $Id: atmosphere.c 3121 2015-05-29 15:22:43Z josef.gasteiger $
 * 
...
int setup_altitude (input_struct input, output_struct *output)
{
  ...
  for (lc=0; lc<output->atm.nlev; lc++) {
    fact = output->atm.microphys.dens[MOL_O2][lc]* 1e-23; /* O4 cross section is scaled by 1e46 due */
                                                          /* to float precision limitations         */
                                                          /* account for that here. ak 20110404     */
    output->atm.microphys.dens[MOL_O4][lc] = fact*fact;
  }
...
/*   } */
  ...
  if ( input.atm.nz_atm_forced_sea != NOT_DEFINED_INTEGER ) {
    ...
    status = interpolate_atmosphere (output->atm.zd,
				     ...
				     output->atm.nlev,
				     input.atm.z_atm_forced_sea, input.atm.nz_atm_forced_sea,
				     input.atm.interpol_method_press,
				     input.atm.interpol_method_temper,
				     input.atm.interpol_method_gas,
				     input.quiet);
    ...
    status = check_hydrostatic_equation ( output->atm.zd, output->atm.microphys.press, output->atm.microphys.temper, output->atm.nlev,
...
int setup_atmosphere (input_struct input, output_struct *output)
{
  ...
  if ( input.atm.ECMWF_ozone_climatology ) {
    status = read_ECMWF_ozone_climatology (input.filename[FN_PATH], input.latitude, input.UTC,
                                           output->atm.microphys.dens[MOL_AIR], output->atm.nlev, 
                                           output->atm.microphys.press, &(output->atm.microphys.dens[MOL_O3]), 
                                           input.verbose, input.quiet );
  ...
  for (lc=0; lc<output->atm.nlev; lc++)
    dens_air_atm[lc] = output->atm.microphys.dens[MOL_AIR][lc];
  ...
  /* Pressure */ 
  status = interpolate_profile (z_atm, &(output->atm.microphys.press), output->atm.nlev, 
				z_all, nlev_all, input.atm.interpol_method_press, input.quiet);
  if (status!=0) {
  ...
  /* Temperature */
  status = interpolate_profile (z_atm, &(output->atm.microphys.temper), output->atm.nlev, 
				z_all, nlev_all, input.atm.interpol_method_temper, input.quiet);
  if (status!=0) {
  ...
  if ( output->atm.microphys.temper_cpt != NOT_DEFINED_FLOAT ) {
    ...
    case OUTLEVEL_ZOUT_ABOVE_SEA:
      for (lc=0; lc<nlev_all; lc++) {
        if ( z_all[lc] == output->atm.microphys.z_cpt_sea ) {
          output->atm.microphys.temper[lc] = output->atm.microphys.temper_cpt;
          break;
        }
      }
      break;
    case OUTLEVEL_PRESS:
      for (lc=0; lc<nlev_all; lc++) {
        if ( fabs(output->atm.microphys.press[lc] - output->atm.microphys.press_cpt) < 1.e-6*output->atm.microphys.press_cpt ) {
          output->atm.microphys.temper[lc] = output->atm.microphys.temper_cpt;
          break;
        }
      }
  ...
  status = interpolate_profile (z_atm, &(output->atm.microphys.dens[MOL_AIR]), output->atm.nlev,
				z_all, nlev_all, input.atm.interpol_method_gas[MOL_AIR], input.quiet);
  if (status!=0) {
  ...
  status = average_dens(output->atm.microphys.temper, output->atm.microphys.dens[MOL_AIR], 
                        z_all, nlev_all,
                        input.atm.interpol_method_temper, 
                        &(output->atm.microphys.temper_avg), YES);
  ...
  for (i=0; i<MOL_NN; i++) {
    ...
    if (i != MOL_AIR ) {
      ...
      status = read_and_combine_density (input.atm.filename[i], &output->atm, 
                                         i, input.atm.interpol_method_gas, input.atm.interpol_method_temper, 
                                         input.atm.unit_profile[i], input.atm.mol_mass,
					 output->atm.microphys.dens[MOL_AIR], output->atm.microphys.temper, z_all, nlev_all,
  ...
  if ((output->atm.microphys.dens_avg[MOL_H2O] = calloc(nlev_all, sizeof(float)))==NULL) {
    fprintf (stderr, "Error: Allocation of memory for dens_avg[MOL_H2O] failed (line %d, function '%s' in '%s')\n", __LINE__, __func__, __FILE__);
    return -1;
  }
  status = column (output->atm.microphys.dens[MOL_H2O], output->atm.zd, output->atm.nlev, output->alt.altitude,
                   input.atm.interpol_method_gas[MOL_H2O], output->atm.microphys.dens[MOL_AIR],
                   &(output->atm.microphys.dens_avg[MOL_H2O]), NO, &(output->column[MOL_H2O]));
  if (status != 0) {
    fprintf (stderr, "Error %d returned by function 'column' (line %d, function '%s' in '%s')\n", status, __LINE__, __func__, __FILE__);
    return status;
  }

  free(output->atm.microphys.dens_avg[MOL_H2O]);
  ...
  /* allocate memory for layer average densities */
  for (i=0; i<MOL_NN; i++) {
    output->atm.microphys.dens_avg[i] = calloc(output->atm.nlyr, sizeof(float));
    if (output->atm.microphys.dens_avg[i] == NULL) {
  ...
  for (i=0; i<MOL_NN; i++) {
    ...
    case(MOL_AIR):
      if (input.pressure >= 0.0) {
        ...
        scale_factor = input.pressure / output->atm.microphys.press[ialt];
        scale_pressure (&(output->atm.microphys.press), &(output->atm.microphys.dens),output->atm.nlev, 
                        scale_factor, input.atm.well_mixed_gas, ialt, input.verbose);
    
      }
      status = column (output->atm.microphys.dens[i], output->atm.zd, output->atm.nlev, output->alt.altitude, 
                       input.atm.interpol_method_gas[i], output->atm.microphys.dens[MOL_AIR],
                       &(output->atm.microphys.dens_avg[i]), NO, &(output->column[i]));  /* in atmosphere.c */
      if (status!=0) {
    ...
    case(MOL_O3):
    case(MOL_O2):
    /* H20 is a special case, see below */
    case(MOL_CO2):
    case(MOL_NO2):
    case(MOL_BRO):
    case(MOL_OCLO):
    ...
    case(MOL_N2):
      ...
      status = scale_density_profile (&(output->atm.microphys.dens[i]), 
                      &(output->atm.microphys.dens_avg[i]), YES, i,
      ...
      if (i == output->atm.microphys.denstab_id) {
        ...
        for (is=0; is<output->atm.microphys.nsza_denstab; is++) {
          ...
          status = scale_density_profile (&(output->atm.microphys.denstab[i][is]), 
                          &(output->atm.microphys.denstab_avg[i][is]), NO, i,
                          input.atm.column[i], &(output->column_denstab[i][is]),
                          &(output->scale_factor_denstab[i][is]), input.atm.interpol_method_gas[i],
                          input, output);   /* in atmosphere.c */
    ...
    case MOL_H2O:
      ...
      status = column (output->atm.microphys.dens[i], output->atm.zd, output->atm.nlev, output->alt.altitude,
                       input.atm.interpol_method_gas[i], output->atm.microphys.dens[MOL_AIR],
                       &(output->atm.microphys.dens_avg[i]),YES,&(output->column[i]));  /* in atmosphere.c */
      if (status!=0) {
        fprintf (stderr, "\nError %d returned by column()\n", status);
        fprintf (stderr, "      during calculation of %s column (line %d, function '%s' in '%s')\n", gas, __LINE__, __func__, __FILE__);
        return status;
	...
	for (iscale=0;iscale<20;iscale++) { 
	  ...
	  for (lc = 0; lc<output->atm.nlev; ++lc) {
	    ...
	    output->atm.microphys.dens[i][lc] *= output->scale_factor[i];
	  ...
	  status = column (output->atm.microphys.dens[i], output->atm.zd, output->atm.nlev, output->alt.altitude, 
			   input.atm.interpol_method_gas[i], output->atm.microphys.dens[MOL_AIR],
			   &(output->atm.microphys.dens_avg[i]),NO,&(output->column[i]));
  ...
  if (strlen(input.filename[FN_MOL_TAU_ABS]) > 0) {
    ...
    /* FIXCE should be no longer necessary here, to be removed after some more testing */ 
/*     if(input.rte.mc.spectral_is){ */
/*       /\* MYSTIC spectral calculation should be done for all wavelengths in the molecular_tau_file. *\/ */
      
/*       /\* if rayleigh_tau_file is also specified, the wavelengths in*\/ */
/*       /\* molecular_tau_file have to be the same  as in rayleigh_tau_file*\/ */
/*       if(strlen(input.filename[FN_RAYLEIGH]) > 0 ){ */
/*         if(tmp_nlambda !=output->mc.alis.nlambda_abs){ */
/*           fprintf(stderr, "Error: if mc_spectral_is is specified together with rayleigh_tau_file \n"); */
/*           fprintf(stderr, "       the wavelength grid in the rayleigh_tau_file must be identical to \n"); */
/*           fprintf(stderr, "       the one in molecular_tau_file. \n"); */
/*           return -1; */
/*         }  */
/*         for (iv=0; iv<tmp_nlambda; iv++){ */
/*           if (output->mc.alis.lambda[iv]!=tmp_lambda[iv]){ */
/*             fprintf(stderr, "Error: if mc_spectral_is is specified together with rayleigh_tau_file \n"); */
/*             fprintf(stderr, "       the wavelength grid in the rayleigh_tau_file must be identical to \n"); */
/*             fprintf(stderr, "       the one in molecular_tau_file. \n"); */
/*             return -1; */
...
/*         output->mc.alis.lambda = calloc(output->wl.nlambda_r, sizeof(float)); */
/*       } */
/*       /\* float might be not accurate enough when wavelength resolution is very fine. *\/ */
/*       /\* This could lead to different wavelengths in output file compared to the input molecular_tau_file.*\/ */
/*       for (iv=0; iv<output->wl.nlambda_r; iv++){ */
/*         output->mc.alis.lambda[iv]=tmp_lambda[iv]; */
/*       } */
/*     } */
  ...
  if (input.verbose) {
    ...
    for (lc=0; lc<output->atm.nlev; lc++) {
      if (output->atm.zd[lc] >= output->alt.altitude) {
        ...
        if (output->column[MOL_O4]   !=0)
          fprintf (stderr, "   %.5e",output->atm.microphys.dens[MOL_O4][lc]);
        fprintf (stderr, "\n");
      }
    }
  ...
  if (output->atm.zout_sur[0] != ZOUT_MYSTIC && input.rte.solver!=SOLVER_SSLIDAR) { /* all cases except MYSTIC 2D surface */
    ...
    for (iz=0; iz<output->atm.nlev; iz++)
      output->atm.microphys.dens_zout[MOL_AIR][iz] = output->atm.microphys.dens[MOL_AIR][iz];
    ...
    for (i_gas=0;i_gas<MOL_NN;i_gas++) {
      ...
      if (i_gas != MOL_AIR ) {
        ...
        for (iz=0; iz<output->atm.nlev; iz++)
          output->atm.microphys.dens_zout[i_gas][iz] = output->atm.microphys.dens[i_gas][iz];
        ...
        status = interpolate_density (output->atm.zd, &(output->atm.microphys.dens_zout[i_gas]), output->atm.nlev, 
				      ...
				      output->atm.microphys.dens[MOL_AIR], output->atm.microphys.dens_zout[MOL_AIR], input.quiet);
        if (status!=0) {
    ...
    case OUTLEVEL_MODEL_LEVELS_AND_LAYERS:
      ...
      for (iz=0; iz<output->atm.nlev; iz++)
        output->atm.microphys.press_zout[iz] = output->atm.microphys.press[iz];
        
      /* interpolation, size is adjusted automagically in interpolate_profile */
      status = interpolate_profile (output->atm.zd, &output->atm.microphys.press_zout, output->atm.nlev,
                                    output->atm.zout_sea, output->atm.nzout, 
    ...
    for (iz=0; iz<output->atm.nlev; iz++)
      output->atm.microphys.temper_zout[iz] = output->atm.microphys.temper[iz];
        
    status = interpolate_profile (output->atm.zd, &output->atm.microphys.temper_zout, output->atm.nlev,
                                  output->atm.zout_sea, output->atm.nzout, 
    ...
    for (lc=0;lc<output->atm.nlev;lc++)
      output->atm.microphys.theta[lc] = output->atm.microphys.temper[lc] * 
                                        pow((1000./output->atm.microphys.press[lc]),kappa);
    /* on zout levels */
    for (iz=0;iz<output->atm.nzout;iz++)
  ...
  for (lc=0; lc<output->atm.nlev; lc++) {
    ...
    if (output->atm.microphys.press[lc]/output->atm.microphys.press[output->atm.nlev-1]>1e-6) {
      tmpdens = output->atm.microphys.press[lc]*1E-04 / (boltzmann*output->atm.microphys.temper[lc]);
      
      if ((tmpdens==0 && output->atm.microphys.dens[MOL_AIR][lc]!=0) || 
	  (tmpdens!=0 && output->atm.microphys.dens[MOL_AIR][lc]==0))
	idealgas=0;
      
      if (output->atm.microphys.dens[MOL_AIR][lc]>0)
	if ((idealdiff = fabs ((tmpdens - output->atm.microphys.dens[MOL_AIR][lc]) / output->atm.microphys.dens[MOL_AIR][lc])) > 0.01)
	  idealgas=0;
	...
	fprintf (stderr, "   %7.2f  %.4e      %.4e    %6.1f%%   %10.5f  %8.3f\n", 
		 output->atm.zd[lc], output->atm.microphys.dens[MOL_AIR][lc], tmpdens, idealdiff*100.0, 
                 output->atm.microphys.press[lc], output->atm.microphys.temper[lc]);
      }
    }
  }
...
int scale_profile_with_mixing_ratio(input_struct input, output_struct *output, int i_mx, int i_mol, 
  ...
  if (input.mixing_ratio[i_mx]<0) {
    ...
    if(default_ppm>=0)
      ...
      output->mixing_ratio[i_mx] = output->atm.microphys.dens[i_mol][ialt] / output->atm.microphys.dens[MOL_AIR][ialt] * 1.0e6;
    else{
    ...
    if(i_mol>0){
      ...
      if(input.mixing_ratio[i_mx]>0 && output->atm.microphys.dens[i_mol][ialt]==0){
      ...
      scale_factor = (input.mixing_ratio[i_mx] / 1.0e6) / 
        (output->atm.microphys.dens[i_mol] [ialt] / 
         output->atm.microphys.dens[MOL_AIR][ialt]);
    
      if (!input.quiet)
        fprintf(stderr, " ... scaling %s-mixing ratio from %10.5e ppm to %10.5e ppm\n", mol_name,
	      (output->atm.microphys.dens[i_mol][ialt]/output->atm.microphys.dens[MOL_AIR][ialt])*1e6,
	         input.mixing_ratio[i_mx]); 

      for (lc=0; lc<output->atm.nlev; lc++)
        output->atm.microphys.dens[i_mol][lc] *= scale_factor;

      /* calculate new column and new dens_avg values */
      status = column (output->atm.microphys.dens[i_mol], output->atm.zd, output->atm.nlev, output->alt.altitude, 
                     input.atm.interpol_method_gas[i_mol], output->atm.microphys.dens[MOL_AIR],
                     &(output->atm.microphys.dens_avg[i_mol]), NO, &(output->column[i_mol]));  /* in atmosphere.c */
      if (status!=0) {
        fprintf (stderr, "\nError %d returned by column()\n", status);
                      ...
                      float **z_all, int *nlev_all)
{
  ...
  case OUTLEVEL_ZOUT_ABOVE_SUR:
    for (iz=0;iz<output->atm.nzout_org;iz++) {
      if (input.atm.zout_sur[iz] == ZOUT_MYSTIC) {   /* marker for 2D-SURFACE IN MYSTIC */
        ...
        if ( output->atm.microphys.z_cpt_sea == NOT_DEFINED_FLOAT ) {
          status_CPT = find_cold_point_tropopause ( output->atm.microphys.press, output->atm.microphys.temper, output->atm.zd, output->atm.nlev, 
                                                    &(output->atm.microphys.z_cpt_sea), &(output->atm.microphys.press_cpt), 
                                                    &(output->atm.microphys.temper_cpt), input.quiet, input.verbose );
        }
        if ( status_CPT == 0 ) {
  ...
  case OUTLEVEL_PRESS:
    ...
    for ( lc=0; lc<output->atm.nlev; lc++ ) {
      log_p_atm[lc] = log(output->atm.microphys.press[lc]);
    }
    ...
    for (iz=0;iz<output->atm.nzout_org;iz++) {
      if      (input.atm.press_zout[iz] == ZOUT_MYSTIC)    /* marker for MYSTIC */
        ...
        output->atm.press_zout_org[iz - skipp_CPT] = output->atm.microphys.press[output->atm.nlev-1];
      }
      else if (input.atm.press_zout[iz] == ZOUT_TOA) {     /* marker for TOP_OF_ATMOSPHERE */
        output->atm.press_zout_org[iz - skipp_CPT] = output->atm.microphys.press[0];
      }
...
int setup_gases (input_struct input, output_struct *output)
{
  ...
  double *babsotot_mol;
  double deltaz=0;
  int iq=0, is=0, isthis=0, iv=0, lc=0, ipa=0;
  int nlev = output->atm.nlev;
  ...
  if (first!=0) {
    ...
    if ((status = ASCII_calloc_double_3D_arylen (&output->atm.optprop.tau_rayleigh_r, 
    ...
    if ((status = ASCII_calloc_double_3D_arylen (&output->atm.optprop.tau_molabs_r, 
        					 nlyr, output->wl.nlambda_r, 
        					 output->atm.nq_r)) != 0)   {
      if (status!=0) {
	fprintf (stderr, "Error %d returned by ASCII_calloc_double_3D_arylen in %s (%s) allocating output->atm.optprop.tau_molabs_r\n",
		 status, function_name, file_name); 
	return status;
      }
    }
    ...
    if ((status = ASCII_calloc_double_3D_arylen (&output->atm.optprop.tau_molabs_md_r, 
						 nlyr, output->wl.nlambda_r, 
						 output->atm.nq_r)) != 0) {
      if (status!=0) {
	fprintf (stderr, "Error %d returned by ASCII_calloc_double_3D_arylen in %s (%s) allocating output->atm.optprop.tau_molabs_md_r\n",
		 status, function_name, file_name); 
	return status;
      }
    } 
	...
	/* average air density over a layer assuming  dens = exp (-c * z) */
        output->atm.cdento[lc] = log_average(output->atm.microphys.dens[MOL_AIR][lc],
                                             output->atm.microphys.dens[MOL_AIR][lc+1]) * deltaz;
      }
    }
  ...
  if (output->atm.microphys.denstab_id) {
    ...
    for (lc=0; lc<nlyr; lc++) { 
      output->atm.microphys.dens[output->atm.microphys.denstab_id][lc] = 
  ...
  for (iv=output->wl.nlambda_rte_lower; iv<=output->wl.nlambda_rte_upper; iv++) {
	...
	case RAYLEIGH_CALC:
	  if (input.ck_scheme != CK_FU) 
	    bscar = output->atm.cdento[lc] * output->crs.crs[0][MOL_AIR][iv];
          else 
	...
	output->atm.optprop.tau_rayleigh_r[lc][iv][iq] = bscar;
      }
    }
  } /* End wavelength loop */
    ...
    for (iv=output->wl.nlambda_rte_lower; iv<=output->wl.nlambda_rte_upper; iv++) {
      ...
      case CK_REPTRAN_CHANNEL:
        ...
        for (lc=0; lc<nlyr; lc++) {
          ...
          case MOLABS_LOOKUP: 
            ...
            for (gas_nr = 1; gas_nr < MOL_NN; gas_nr++) {
              ...
              density = output->atm.microphys.dens_avg[gas_nr][lc];
              
              babso_mol[gas_nr] += deltaz * 0.5 * (output->crs.crs[lc][gas_nr][iv] + output->crs.crs[lc+1][gas_nr][iv]) * density;
              ...
              babso += babso_mol[gas_nr];

              if (output->atm.microphys.denstab_id != gas_nr)  
                babso_md += babso_mol[gas_nr];
              ...
              if (output->atm.microphys.denstab_id == gas_nr) {
                ...
                case INTERP_METHOD_LOG_SPLINE: 
                  density = linmix_average(output->atm.microphys.denstab[gas_nr][isthis][lc],
                                           output->atm.microphys.denstab[gas_nr][isthis][lc+1],
                                           output->atm.microphys.dens[MOL_AIR][lc],
                                           output->atm.microphys.dens[MOL_AIR][lc+1]);
                  break;
                default:
                ...
                babso_amf += deltaz * 0.5 * (output->crs.crs[lc][gas_nr][iv] +
                                             output->crs.crs[lc+1][gas_nr][iv]) * density;
                vertical_column += deltaz * density;
              }
            }
            
            output->atm.optprop.tau_molabs_r [lc][iv][iq] = babso;
	    if ( input.rte.solver == SOLVER_SDISORT) 
	      output->atm.optprop.tau_molabs_md_r[lc][iv][iq] = babso_md;
            break;
          ...
          if (input.verbose) {
            bscar=output->atm.optprop.tau_rayleigh_r[lc][iv][iq];
            bscartot+=bscar;
          
            babsotot      += babso;
        
            for (gas_nr = 1; gas_nr < MOL_NN; gas_nr++) 
              babsotot_mol[gas_nr]   += babso_mol[gas_nr];
            ...
            if (output->atm.zd[lc+1] >= output->alt.altitude) {
              ...
              for (gas_nr = 1; gas_nr < MOL_NN; gas_nr++) 
                fprintf (stderr, " | %10.5e", babso_mol[gas_nr]);
              fprintf(stderr, "\n");
	    }
        ...
        if (input.verbose) {
          ...
          for (gas_nr = 1; gas_nr < MOL_NN; gas_nr++) 
            fprintf (stderr, " | %10.5e", babsotot_mol[gas_nr]);
          fprintf(stderr, "\n");
      ...
      case CK_KATO2_96:
	    ...
	    output->crs.crs[lc][MOL_CO2][iv] = 
	      output->crs_ck.profile[KATO_CO2-1][iv].crs[lc][iq1]; 
	      ...
	      output->crs.crs[lc][MOL_O3][iv]  = 
		output->crs_ck.profile[KATO_O3-1][iv].crs[lc][iq2];
		...
		output->crs.crs[lc][MOL_O2][iv]  =
		  output->crs_ck.profile[KATO_O2-1][iv].crs[lc][iq3];
		  ...
		  output->crs.crs[lc][MOL_H2O][iv]  = 
		    output->crs_ck.profile[KATO_H2O-1][iv].crs[lc][iq4];
                  ...
                  /* O3 absorption */
                  babso += deltaz * output->crs.crs[lc][MOL_O3][iv]  * output->atm.microphys.dens_avg[MOL_O3][lc];
  		  ...
  		  /*         0.5 * (output->atm.microphys.dens[MOL_O3][lc]  + output->atm.microphys.dens[MOL_O3][lc+1]);  */   /* before 2006-03-06, UH */
                  ...
                  /* O2 absorption */
                  babso += deltaz * output->crs.crs[lc][MOL_O2][iv]  * output->atm.microphys.dens_avg[MOL_O2][lc]; 
                  ...
                  /* H2O absorption */
                  babso += deltaz * output->crs.crs[lc][MOL_H2O][iv] * output->atm.microphys.dens_avg[MOL_H2O][lc]; 
     		  /* babso += deltaz * output->crs.crs[lc][MOL_H2O][iv] *                                                 */                              
                  ...
                  /* CO2 absorption */
                  babso += deltaz * output->crs.crs[lc][MOL_CO2][iv] * output->atm.microphys.dens_avg[MOL_CO2][lc]; 
            ...
            for (iq2=0; iq2<nq2; iq2++) { /* O3 */
              for (iq3=0; iq3<nq3; iq3++) { /* O2 */
                for (iq4=0; iq4<nq4; iq4++) { /* H2O */
                  ...
                  for (lc=0; lc<output->atm.nlyr; lc++) {
                    bscartot += output->atm.optprop.tau_rayleigh_r[lc][iv][iq];
                    babsotot += output->atm.optprop.tau_molabs_r  [lc][iv][iq];
                    if (output->atm.zd[lc+1] >= output->alt.altitude) {
                      fprintf (stderr, "%5d | %8.2f | %11.6f | %11.6f |\n", lc, output->atm.zd[lc+1], 
                               output->atm.optprop.tau_rayleigh_r[lc][iv][iq], output->atm.optprop.tau_molabs_r[lc][iv][iq]);
                    }
                  }
                  fprintf(stderr, " --------------------------------------------\n");
	...
	nq = output->crs_ck.profile[0][iv].ngauss;
        ...
        for (iq=0; iq<nq; iq++) {
          ...
          for (lc=0; lc<output->atm.nlyr; lc++) {
            ...
            output->atm.optprop.tau_molabs_r[lc][iv][iq] =
              deltaz/1e5 * output->crs_ck.profile[0][iv].crs[lc][iq];
          }
...
int setup_temperature (input_struct input, output_struct *output)
{
  ...
  for (lev=0; lev<output->atm.nlev; lev++) {
    output->atm.temper_out[lev] = output->atm.microphys.temper [lev];
  }
...

{
  ...
  if (cdf==0) {   /* classic ASCII atmospheric model */
    ...
    out->microphys.press  = ASCII_column_float (atm_data, nlev, ATMFILE_PRESS);
    out->microphys.temper = ASCII_column_float (atm_data, nlev, ATMFILE_TEMPER);
    ...
    if (min_columns>3) { /* air */
      out->microphys.dens[MOL_AIR] = ASCII_column_float(atm_data, nlev, ATMFILE_AIR);
    }
    else {
      out->microphys.dens[MOL_AIR] = (float *) calloc (nlev, sizeof(float));
      
      /* Set air density */
      for (lev=0; lev<nlev; lev++)
	out->microphys.dens[MOL_AIR][lev] = out->microphys.press[lev]*1E-04/(boltzmann*out->microphys.temper[lev]);
    }
    
    if (min_columns>4) /* o3   */
      out->microphys.dens[MOL_O3]  = ASCII_column_float(atm_data, nlev, ATMFILE_O3);
    else
      out->microphys.dens[MOL_O3]  = (float *) calloc (nlev, sizeof(float));
    
    if (min_columns>5)  /* oxygen           */
      out->microphys.dens[MOL_O2]  = ASCII_column_float(atm_data, nlev, ATMFILE_O2);
    else 
      out->microphys.dens[MOL_O2]  = (float *) calloc (nlev, sizeof(float));
    
    
    if (min_columns>6)  /* water vapour     */
      out->microphys.dens[MOL_H2O] = ASCII_column_float(atm_data, nlev, ATMFILE_H2O);
    else 
      out->microphys.dens[MOL_H2O]  = (float *) calloc (nlev, sizeof(float));
    
    
    if (min_columns>7)  /* carbon dioxide   */
      out->microphys.dens[MOL_CO2] = ASCII_column_float(atm_data, nlev, ATMFILE_CO2);
    else 
      out->microphys.dens[MOL_CO2]  = (float *) calloc (nlev, sizeof(float));
    
    
    if (min_columns>8)  /* nitrogen dioxide */
      out->microphys.dens[MOL_NO2] = ASCII_column_float(atm_data, nlev, ATMFILE_NO2);
    else 
      out->microphys.dens[MOL_NO2]  = (float *) calloc (nlev, sizeof(float));


    /* Allocate memory for species not in atmosphere file */

    out->microphys.dens[MOL_BRO]  = (float *) calloc (nlev, sizeof(float));
    out->microphys.dens[MOL_OCLO] = (float *) calloc (nlev, sizeof(float));
    out->microphys.dens[MOL_HCHO] = (float *) calloc (nlev, sizeof(float));
    out->microphys.dens[MOL_O4]   = (float *) calloc (nlev, sizeof(float));
    out->microphys.dens[MOL_SO2]  = (float *) calloc (nlev, sizeof(float));
    out->microphys.dens[MOL_CH4]  = (float *) calloc (nlev, sizeof(float));
    out->microphys.dens[MOL_N2O]  = (float *) calloc (nlev, sizeof(float));
    out->microphys.dens[MOL_CO]   = (float *) calloc (nlev, sizeof(float));
    out->microphys.dens[MOL_N2]   = (float *) calloc (nlev, sizeof(float));
    ...
    /* read "pressure" */
    status = nc_get_vara_float (ncid, id_press, start, count, out->microphys.press);
    if (status!=NC_NOERR) {
    ...
    /* read "temperature" */
    status = nc_get_vara_float (ncid, id_temper, start, count, out->microphys.temper);
    if (status!=NC_NOERR) {
    ...
    /* read "density" */
    status = nc_get_vara_float (ncid, id_air, start, count, out->microphys.dens[MOL_AIR]);
    if (status!=NC_NOERR) {
    ...
    /* read "o3" */
    status = nc_get_vara_float (ncid, id_o3, start, count, out->microphys.dens[MOL_O3]);
    if (status!=NC_NOERR) {
    ...
    /* read "o2" */
    status = nc_get_vara_float (ncid, id_o2, start, count, out->microphys.dens[MOL_O2]);
    if (status!=NC_NOERR) {
    ...
    /* read "h2o" */
    status = nc_get_vara_float (ncid, id_h2o, start, count, out->microphys.dens[MOL_H2O]);
    if (status!=NC_NOERR) {
      fprintf (stderr, "Error %d reading h2o from %s\n", status, filename);
      fprintf (stderr, "      (line %d, function %s in %s)\n",  __LINE__, __func__, __FILE__);
      return status;
    ...
    /* read "co2" */
    status = nc_get_vara_float (ncid, id_co2, start, count, out->microphys.dens[MOL_CO2]);
    if (status!=NC_NOERR) {
  ...
  status = check_hydrostatic_equation ( z_atm, out->microphys.press, out->microphys.temper, nlev,
  ...
  for (lev=0; lev<out->nlev; lev++) {
    ...
    if (out->microphys.press[lev]/out->microphys.press[nlev-1]>1e-6) {
      tmpdens = out->microphys.press[lev]*1E-04 / (boltzmann*out->microphys.temper[lev]);
      
      if ((tmpdens==0 && out->microphys.dens[MOL_AIR][lev]!=0) || 
	  (tmpdens!=0 && out->microphys.dens[MOL_AIR][lev]==0))
	idealgas=0;
      
      if (out->microphys.dens[MOL_AIR][lev]>0)
	if ((idealdiff = fabs ((tmpdens - out->microphys.dens[MOL_AIR][lev]) / out->microphys.dens[MOL_AIR][lev])) > 0.01)
	  idealgas=0;
	...
	fprintf (stderr, "***    z [km]  density [cm-3]  p/kT [cm-3]  difference  press[hPa]  T[K]\n");
      ...
      if (!idealgas) {
	fprintf (stderr, "***   %7.2f  %.4e      %.4e    %6.1f%%  %10.5f  %8.3f\n", 
		 out->zd[lev], out->microphys.dens[MOL_AIR][lev], tmpdens, idealdiff*100.0, 
                 out->microphys.press[lev], out->microphys.temper[lev]);
      }
    }
  }
...

{
  ...
  if (p_rs[0] == 0.0) {
    p_zero=TRUE;
    p_rs[0] = output->atm.microphys.press[0];
  }
...
/*     fprintf (stderr," ### %3d atm p=%e T=%e, z=%e\n", lc, p_rs[lc], T_rs[lc], output->atm.z_model_level[lc] ); */
  ...
  for (gas_nr = 0; gas_nr < MOL_NN; gas_nr++) {
    ...
    case MOL_N2:
      ...
      /* copy number density */
      for (lc=0; lc<output->atm.nlev;lc++)
        dens_rs[gas_nr][lc] = output->atm.microphys.dens[gas_nr][lc]; 
      ...
      if ( input.atm.well_mixed_gas[gas_nr] == YES ) {
        ...
        status = interpolate_density (output->atm.zd, &(dens_rs[gas_nr]), output->atm.nlev, 
				      ...
				      output->atm.microphys.dens[MOL_AIR], dens_rs[MOL_AIR], input.quiet);
        if (status!=0) {
        ...
        status = interpolate_density (output->atm.zd, &(dens_rs[gas_nr]), output->atm.nlev, 
				      output->atm.z_model_level, output->atm.nz_model_level, input.atm.interpol_method_gas[gas_nr],
				      output->atm.microphys.dens[MOL_AIR], dens_air_atm, input.quiet);
        if (status!=0) {
          fprintf (stderr, "Error %d interpolating dens_rs[%d]\n", status, gas_nr);
          fprintf (stderr, "      (line %d, function '%s' in '%s')\n", __LINE__, __func__, __FILE__ );
          return status;
  ...
  if (input.atm.rs_add_upper_levels) {
    ...
    if (n_comb-1-output->atm.nz_model_level >= 0) {
      ...
      T_comb[lc] = output->atm.microphys.temper[n_above-1];
      ...
      scale_factor = p_comb[lc] / output->atm.microphys.press[n_above-1];
      scale_pressure (&(output->atm.microphys.press), &(output->atm.microphys.dens),output->atm.nlev,
                    scale_factor, input.atm.well_mixed_gas, n_above-1, FALSE);

      for (gas_nr = 0; gas_nr < MOL_NN; gas_nr++) 
        dens_comb[gas_nr][lc] = output->atm.microphys.dens[gas_nr][n_above-1];
      ...
      for (lc=n_above-2;lc>=0;lc--) {
        p_comb[lc] = output->atm.microphys.press[lc];
        T_comb[lc] = output->atm.microphys.temper[lc];
        z_comb[lc] = output->atm.zd[lc];
        for (gas_nr = 0; gas_nr < MOL_NN; gas_nr++)
          dens_comb[gas_nr][lc] = output->atm.microphys.dens[gas_nr][lc];
          /* fprintf (stderr,"%3d %7.3f %11.5f %7.2f %13.6e %13.6e %13.6e %13.6e %13.6e %13.6e %13.6e %13.6e %13.6e\n", */
          /*                 lc, z_comb[lc], p_comb[lc], T_comb[lc], */
          /*                 dens_comb[0][lc],dens_comb[1][lc],dens_comb[2][lc],dens_comb[3][lc],dens_comb[4][lc], */
          /*                 dens_comb[5][lc],dens_comb[6][lc],dens_comb[7][lc],dens_comb[8][lc]); */
  ...
  free(output->atm.zd);
  free(output->atm.microphys.press);
  free(output->atm.microphys.temper);
  for (gas_nr = 0; gas_nr < MOL_NN; gas_nr++)
    free(output->atm.microphys.dens[gas_nr]);
  ...
  if ((output->atm.microphys.press  = (float *) calloc (n_comb, sizeof(float))) == NULL) {
    fprintf (stderr, "Error allocating memory for output->atm.microphys.press\n");
    return -13;
  }

  if ((output->atm.microphys.temper = (float *) calloc (n_comb, sizeof(float))) == NULL) {
    fprintf (stderr, "Error allocating memory for output->atm.microphys.temper\n");
    return -14;
  }

  for (gas_nr = 0; gas_nr < MOL_NN; gas_nr++)
    if ((output->atm.microphys.dens[gas_nr] = (float *) calloc (n_comb, sizeof(float))) == NULL) {
      fprintf (stderr, "Error allocating memory for output->atm.zd\n");
      return -15;
    }
  ...
  for (lc=0;lc<n_comb;lc++) {
    ...
    output->atm.microphys.press         [lc] = p_comb[lc];
    output->atm.microphys.temper        [lc] = T_comb[lc];
    for (gas_nr = 0; gas_nr < MOL_NN; gas_nr++)
      output->atm.microphys.dens[gas_nr][lc] = dens_comb[gas_nr][lc];
  }
    ...
    if (strlen(filename) > 0) {
	  ...
	  return status; 
        ...
        status = combine_profiles (dens, zd, nlev, 1,
                                   out->microphys.dens[dens_id], dens_air_all, out->zd, out->nlev,
	  ...
	  if (status!=0) {
        ...
        out->microphys.dens[dens_id] = (float *) calloc (nlev_all, sizeof (float));
    
        for (lc=0; lc<nlev_all; lc++)
          out->microphys.dens[dens_id][lc] = dens_comb[0][lc];
...
/*           fprintf (stderr, "lc = %d, z_comb = %5.2f, dens_comb %11.7e\n", lc, z_comb[lc], dens_comb[0][lc]); */
	  ...
	  if (status!=0) {
        ...
        if (isthis < 0) {
          ...
          status = interpolate_density (z_atm, &(out->microphys.dens[dens_id]), nlev_atm, 
                                         z_all, nlev_all, interpol_method_gas[dens_id],
          ...
          for (lc=0; lc<nlev_all; lc++) {
            out->microphys.dens[dens_id][lc] = out->microphys.denstab[dens_id][isthis][lc];
          }
        }
...
static int scale_pressure (float **pressure, float ***dens, int nlev, float scale_factor, int *well_mixed_gas, int ialt, int verbose)
{
  ...
  for (lc=0;lc<nlev;lc++)
    (*pressure)[lc] *= scale_factor;
  if (verbose)
    fprintf (stderr, " ... scaling pressure    from  %11.6f hPa   to %11.6f hPa\n", 
	     (*pressure)[ialt]/scale_factor, (*pressure)[ialt]);
  ...
  for (gas_nr=0;gas_nr<MOL_NN;gas_nr++)
    if ( well_mixed_gas[gas_nr] == YES ) { 
      ...
      for (lc=0;lc<nlev;lc++) 
        (*dens)[gas_nr][lc] *= scale_factor;
      if (verbose)
        fprintf (stderr, " ... scaling %s density from %11.5g cm-3  to %11.5g cm-3\n", 
	                       gas, (*dens)[gas_nr][ialt]/scale_factor, (*dens)[gas_nr][ialt]);
    }
                          ...
                          input_struct input, output_struct *output)
{
  ...
  status = column ((*dens), output->atm.zd, output->atm.nlev, output->alt.altitude, 
                   interpol_method_gas, output->atm.microphys.dens[MOL_AIR],
...
static int calculate_dtheta_dz (input_struct input, output_struct *output) 
{
  ...
  for (iz=0; iz<output->atm.nlev; iz++)
    theta[iz] = output->atm.microphys.temper[iz] * pow((1000./output->atm.microphys.press[iz]),kappa);
