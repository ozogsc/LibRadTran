...
int singleScatteringRadar ( ssRadarStruct* ssRadar, char* inputFilename, char* outputFilename )
{
  ...
  allLayersReflectivities = calloc(numberOfAllLayers,sizeof(double));
  ...
  *dB = 1;
  ...
  if (*outputFilename != *ssRadarOutputFilename) {
    ...
    if (output < 1) {
      ...
      fprintf(ssRadarOutputFile,"First Range Gate Distance: %e m\n", rangeGates[0] );
      fprintf(ssRadarOutputFile,"First Range Gate Height: %e m\n", rangeGatesVertical[0] );
      fprintf(ssRadarOutputFile,"No \t RG Distance \t RG Height \t Z\n" );
      for (i=0;i<numberOfRangeGates;i++)
  ...
  fprintf(stdout,"First Range Gate Distance: %e m\n", rangeGates[0] );
  fprintf(stdout,"First Range Gate Height: %e m\n", rangeGatesVertical[0] );
  fprintf(stdout,"No \t RG Distance \t RG Height \t Z\n" );
  for (i=0;i<numberOfRangeGates;i++)
