/*--------------------------------------------------------------------
 * $Id: ck.c 2887 2013-03-11 09:19:28Z robert.buras $
 * 
...
static int x_section (int scheme, int ispcs, 
                      float *press, float *temp_avg, int nlev, 
                      int iband, char *path, int *ngauss, double **xabs, double *weight, int quiet, int verbose);

static int x_section_h2o (int scheme, float *press, float *temp_avg, float **dens, float **dens_avg,
                          int nlev, int iband, char *path, 
                          int *ngauss, double **xabs, double *weight, int quiet, int verbose);

int hunt(float *xx, int n, double *x, int *jlo);

static int kato_crs (float *temp_avg, float *press, float **dens, float **dens_avg, float *z, int nlev,
                     ck_struct *crs, int iband, int ispcs, char *path, int quiet, int verbose,
                     double **extinction, double *weight, int *ngauss);

static int fu_profile (float *temper, float *press, float *z, float **dens, float **dens_avg, int nlev,
...
static int avhrr_kratz_profile (float *temper, float *press, float *z, float **dens, int nlev, 
                                float umco2, float umch4, float umn2o, float umf11, float umf12,
                                ck_profile *ck);
  ...
  if (crs->comp[ispcs][iband] < 0) {
    ...
    *ngauss = crs->wght[ispcs][iband];
    ...
    if (ispcs == KATO_H2O) {
      if (iband==9 || iband==10 || iband==11 || iband==13 || iband == 15) {
        status = x_section (crs->scheme, ispcs, press, temp_avg, nlev, iband, 
        ...
        status = x_section_h2o (crs->scheme, press, temp_avg, dens, dens_avg, nlev, iband, 
                                path, ngauss, extinction, weight, quiet, verbose);
        if (status!=0) {
    ...
    *ngauss = 1;
    ...
    for (lc=0; lc<nlev-1; lc++)
      extinction[lc][0] = crs->comp[ispcs][iband];
  }
                       ...
                       ck_profile *ck)
{
  ...
  for (lc=0; lc<nlev; lc++) {
    rhoair[lc] = dens[MOL_AIR][lc] * M_AIR / N_MOL_CKDFU * 1.0e6;
    rhoh2o[lc] = dens[MOL_H2O][lc] * M_H2O / N_MOL_CKDFU * 1.0e6;
    rhoo3 [lc] = dens[MOL_O3] [lc] * M_O3  / N_MOL_CKDFU * 1.0e6;
    temp  [lc] = temper[lc];
    pres  [lc] = press[lc];
  }

  for (lc=0; lc<nlev-1; lc++)
    davg  [lc] = dens_avg[MOL_AIR] [lc] * M_AIR  / N_MOL_CKDFU * 1.0e6 * (z[lc]-z[lc+1]);
  ...
  status = ckdfu (umco2, umch4, umn2o,
                  umf11, umf12, umf22,
                  rhoair, rhoh2o, rhoo3, pres, temp, davg, z, nlev, h2o_cont,
                  ck);
#else
                                ...
                                ck_profile *ck)
{
  ...
  for (lc=0;lc<nlev;lc++) {
    ...
    p0[nlev-lc-1] = press[lc]/1013.25;   /* convert pressure from mbar to atm */
    t0[nlev-lc-1] = temper[lc];

    u0[nlev-lc-1] = dens[MOL_H2O][lc] * H2O_MOLWGHT / NAVOGADRO * 1.0E5;
    ux[nlev-lc-1] = dens[MOL_O3] [lc] * O3_MOLWGHT  / NAVOGADRO * 1.0E5;
                    ...
                    int spectral_is)
{
  ...
  if (nlev > ck[iv].nlev) {
    ...
    ck[iv].crs = calloc (nlev, sizeof(double *));
    for (lc=0; lc<nlev; lc++)
      ck[iv].crs[lc] = calloc (LOWTRAN_MAXINT, sizeof(double));
  ...
  for (lc=0; lc<nlev; lc++) {
    zi[lc] = z      [nlev-lc-1];
    pi[lc] = press  [nlev-lc-1];
    ti[lc] = temper [nlev-lc-1];
    wh[lc] = h2o    [nlev-lc-1] * 1.0e6 * (M_H2O / N_MOL_CKDFU);
    wo[lc] = o3     [nlev-lc-1] * 1.0e6 * (M_O3  / N_MOL_CKDFU);
  }
  ...
  if (spectral_is && npass==1)
    for (ig=0; ig<LOWTRAN_MAXINT; ig++){
      ...
      for (lc=0; lc<nlev-1; lc++)
        ck[iv].crs[lc][ig] = dtauc[0][lc+1] / (z[lc]-z[lc+1]);
    } 
  else{
    for (ig=0; ig<npass; ig++) {
      ...
      for (lc=0; lc<nlev-1; lc++)
        ck[iv].crs[lc][ig] = dtauc[ig][lc+1] / (z[lc]-z[lc+1]);
    }
  }
                            ...
                            ck_profile *ck)
{
  ...
  for (iv=0; iv<nwvn; iv++) {
    ...
    for (lu=0; lu<nlev-1; lu++) {
      ...
      status = nc_get_vara_double (ncid, id_k, crsstart, crscount, ck[iv].crs[lu]);
      if (status!=NC_NOERR) {
        fprintf (stderr, "Error %d reading %s\n", status, filename);
        return status;
      }
            ...
            input_struct input, output_struct *output)
{
  ...
  int is=0, lc=0, iv=0, ipa=0, status=0;
    ...
    case CK_LOWTRAN:
      for (is=0; is<n_components; is++) {
        for (iv=0; iv<n_lambda; iv++) {
          crs_ck->profile[is][iv].crs    = calloc (nlev, sizeof(double *));
          crs_ck->profile[is][iv].weight = calloc (n_maxint, sizeof(double));
            
          for (lc=0; lc<nlev; lc++)
            crs_ck->profile[is][iv].crs[lc] = calloc(n_maxint, sizeof(double));
        }
      }
      break;
    case CK_FILE:
      for (is=0; is<GENERIC_COMPONENTS; is++) {
        for (iv=0; iv<n_lambda; iv++) {
          ...
          for (lc=0; lc<nlev; lc++)
            crs_ck->profile[is][iv].crs[lc] = calloc (ck->wght[GENERIC_WGHT][iv+1], sizeof(double));
        }
      }
      break;
    default:
  ...
  for (ipa=0; ipa<nipa; ipa++) {
    ...
    for (iv=0; iv<n_lambda; iv++) {
        ...
        /* CO2 */ 
        status = kato_crs (temp_avg, press, dens, dens_avg, z, nlev,  
    ...
    case CK_FU:
      ...
      status = fu_profile (temp, press, z, dens, dens_avg, nlev, h2ocont,
                           mixing_ratio[MX_CO2],
                           mixing_ratio[MX_CH4], 
                           mixing_ratio[MX_N2O], 
                           mixing_ratio[MX_F11], 
...
static int x_section (int scheme, int ispcs, float *press, float *temp_avg, int nlev, 
                      int iband, char *path, int *ngauss, double **xabs, double *weight, int quiet, int verbose)
{
  int status=0;
#if HAVE_LIBNETCDF
  ...
  int w_id=0, p_id=0, t_id=0, ncid=0, i=0, k=0, lc=0;
...
/* The unit of the cross section is cm**2 per molecule.                            */
  ...
  for (lc=0; lc<nlev-1; lc++) {
    ...
    pressure    = log_average(press[lc], press[lc+1]);         /* [mbar] */   /* double <- float */  
    /* pressure    = 0.5 * ( press[lc] + press[lc+1] ); */     /* [mbar] */   /* double <- float */  /* before 2006-03-06, UH */
    temperature = temp_avg[lc];                                /* [K]    */   /* double <- float */
    ...
    for (k = 1; k <= *ngauss; ++k) {
      xabs[lc][k] = ofrcnx *ofrcny * xsect[(k << 2) - 4] + frcnx * 
                          ...
                          int *ngauss, double **xabs, double *weight, int quiet, int verbose)
{
  ...
  int w_id=0, p_id=0, t_id=0, r_id=0, ncid=0, i=0, k=0, lc=0;
  ...
  for (lc=0; lc<nlev-1; lc++) {
    ...
    /* pressure    = 0.5 * ( press[lc] + press[lc+1] ); */     /* [mbar] */   /* double <- float */  /* before 2006-03-06, UH */
    temperature = temp_avg[lc];                                /* [K]    */   /* double <- float */
    density = dens_avg[MOL_H2O][lc];
    /* density = 0.5 * (dens[MOL_H2O][lc] + dens[MOL_H2O][lc+1]); */                                 /* before 2006-03-06, UH */
    ...
    if (irho <= 10) {
      ...
      for (k = 1; k <= *ngauss; ++k) {
        ...
        xabs[lc][k] = (ansyru - ansyrl) * (rholog - r[irho - 1]) / 
      ...
      for (k = 1; k <= *ngauss; ++k) {
        xabs[lc][k] = ofrcnx * ofrcny * xsect[(k << 3) - 8] + 
          frcnx * ofrcny * xsect[(k << 3) - 7] + frcnx * 
          frcny * xsect[(k << 3) - 6] + ofrcnx * frcny * 
          xsect[(k << 3) - 5];
      }
