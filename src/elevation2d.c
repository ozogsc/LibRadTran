/*--------------------------------------------------------------------
 * $Id: elevation2d.c 3058 2014-07-24 08:42:24Z bernhard.mayer $
 * 
		       ...
		       int visualize, int quiet)
{
  ...
  for (ie=0; ie<=elev->Nx; ie++)
    if (fabs (elevation2D[ie][0] - elevation2D[ie][elev->Ny]) > MC_EPSILON) {
  ...
  for (je=0; je<=elev->Ny; je++)
    if (fabs (elevation2D[0][je] - elevation2D[elev->Nx][je]) > MC_EPSILON) {
      fprintf (stderr, "Error, elevation grid not periodic!\n");
      fprintf (stderr, "elevation2D [%d][%d] = %g, elevation2D [%d][%d] = %g\n",
