/*--------------------------------------------------------------------
 * $Id: aerosol.c 3179 2015-08-24 09:17:36Z Claudia.Emde $
 * 
...
static int scale_aer_king_byrne (float **dtau, float *lambda_t, int nlambda_t, int nlyr,
  ...
  if (input.aer.vulcan > 0) {
    ...
    if (strlen(input.aer.filename[FN_AER_TAU]) > 0) { 
      if (!tau_from_netCDF)
        ...
        status += add_sigma_nc_file2grid (&tmp_zd, &tmp_nlev, input.aer.filename[FN_AER_TAU],
                            input.latitude, input.longitude, input.UTC, input.atm.time_interpolate,
                            output->atm.zd, output->atm.microphys.press, output->atm.nlev,
	...
	status = free_aer_out (tmpaer); 
      ...
      if (strlen(input.aer.filename[FN_AER_TAU]) > 0) {
        if (!tau_from_netCDF) {
          ...
          status = read_aerosol_profile_from_map (input.aer.filename[FN_AER_TAU], input.latitude, input.longitude,  
     ...
     /* fprintf(stderr,"ang alpha %f, ang beta %f, kb alpha_0 %f  kb alpha_1 %f  kb alpha_2 %f \n ", input.aer.alpha,input.aer.beta, input.aer.alpha_0, input.aer.alpha_1, input.aer.alpha_2); */
      ...
      if (input.aer.beta >= 0.0 && input.aer.alpha_2<-800) {
        status = scale_aer_angstrom (output->aer.optprop.dtau, output->wl.lambda_t, output->wl.nlambda_t, 
      ...
      if (input.aer.alpha_2 >= -800.0) {
        status = scale_aer_king_byrne (output->aer.optprop.dtau, output->wl.lambda_t, output->wl.nlambda_t, 
...
static int calculate_relative_humidity_on_aerosol_grid (input_struct input, output_struct *output, float **tmp_rh)
{
  ...
  status = arb_wvn (output->atm.nlev, output->atm.zd, output->atm.microphys.dens[MOL_AIR],
  ...
  for (lc=0; lc<output->atm.nlev; lc++)
    tmp_h2o[lc] = output->atm.microphys.dens[MOL_H2O][lc]; 
  ...
  status += interpolate_density (output->atm.zd, &(tmp_h2o), output->atm.nlev, 
                                 ...
                                 output->atm.microphys.dens[MOL_AIR], tmp_air, input.quiet);
  if (status!=0) {
  ...
  status = arb_wvn (output->atm.nlev, output->atm.zd, output->atm.microphys.temper,
			       ...
			       float alpha_0,float alpha_1, float alpha_2, float *zd, float altitude)
{
  ...
  /* Function Body */
  for (iv=0; iv<nlambda_t; iv++) {
    
    ext = 0.0;
    for (lc=0; lc<=ialt-1; lc++)
      ext += dtau[iv][lc];
    
    depth = exp(alpha_0) * pow(lambda_t[iv]/1000.0, alpha_1) * pow(lambda_t[iv]/1000.0, alpha_2*log(lambda_t[iv]/1000.0));
    fact = depth / ext;
...
int read_aerosol_species_from_netCDF_map ( input_struct input, output_struct *output, char *filename )
{
  ...
  for ( lc=0; lc<output->aer.nlev; lc++ ) {
    p_aer[lc] = 0.01*hyai[lc] + hybi[lc]*output->atm.microphys.press[output->atm.nlev-1]; /* 0.01 == Pa -> hPa */
  ...
  for ( lc=0; lc<aer_nlayer; lc++ ) {
    p_aer_m[lc] = 0.01*hyam[lc] + hybm[lc]*output->atm.microphys.press[output->atm.nlev-1]; /* 0.01 == Pa -> hPa */
  ...
  status = arb_wvn (output->atm.nlev, log_p_atm, output->atm.microphys.dens[MOL_AIR],
