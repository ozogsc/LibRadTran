/*--------------------------------------------------------------------
 * $Id: albedo.c 3155 2015-08-10 10:48:25Z robert.buras $
 * 
...
/*      } */
  ...
  /* 2D spectral BRDF */
  if (strlen(input.rte.mc.filename[FN_MC_RPVTYPE]) > 0) {

    status = read_spectral_BRDFs (input.rte.mc.filename[FN_MC_RPVTYPE], &(output->rpv_surf),
    ...
    if (status!=0) {
      fprintf (stderr, "Error %d returned by read_spectral_BRDFs from %s (line %d, function %s in %s)\n", 
                       status, input.rte.mc.filename[FN_MC_RPVTYPE], __LINE__, __func__, __FILE__);
      return status;
    }
  }
  
  /* 2D spectral albedo */
  if (strlen(input.rte.mc.filename[FN_MC_ALBEDOTYPE]) > 0) { 
    
    status = read_spectral_albedos (input.rte.mc.filename[FN_MC_ALBEDOTYPE], &(output->rpv_surf),
				    output->wl.lambda_r, output->wl.nlambda_r,
				    output->wl.nlambda_rte_lower, output->wl.nlambda_rte_upper,
				    input.quiet);
    if (status!=0) {
      fprintf (stderr, "Error %d returned by read_spectral_albedos from %s (line %d, function %s in %s)\n", 
	       status, input.rte.mc.filename[FN_MC_ALBEDOTYPE], __LINE__, __func__, __FILE__);
      return status;
    }
  }
